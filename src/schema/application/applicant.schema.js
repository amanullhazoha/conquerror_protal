      ...
      if (allowedDomains.includes(domain)) {
        try {
          const response = await fetch(
            ...
            }/api/v1/public/career/jobs/mail-check?email=${value}&id=${id}`
          );
...
Yup.addMethod(Yup.string, "checkRefCodeExist", function (message) {
    ...
    if (value) {
      try {
        const response = await fetch(
          `${
            import.meta.env.VITE_APP_BASE_API_URL
          }/api/v1/public/user/refer-code?code=${value}`
        );
...
export const jobApplyContactInfoSchema = (id) =>
  Yup.object().shape({
    email: Yup.string()
...
export const jobApplyPassportSchema = (id) =>
  Yup.object().shape({
...
export const jobApplyNidSchema = Yup.object().shape({
